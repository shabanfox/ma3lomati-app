...
trans = {
    "EN": {
        ...
        "login_h": "PLATFORM ACCESS", "user": "Username", "pass": "Password", "login_btn": "Sign In", "lang_toggle": "العربية"
    },
    "AR": {
        ...
        "login_h": "بوابة دخول المحترفين", "user": "اسم المستخدم", "pass": "كلمة المرور", "login_btn": "تسجيل الدخول", "lang_toggle": "English"
    }
}
...
st.markdown(f"""
    ...
    .label-gold {{ color: #f59e0b; font-weight: 900; font-size: 18px; margin-top: 15px; }}
    .val-white {{ color: white; font-size: 18px; margin-bottom: 10px; }}
    </style>
...
if not st.session_state.auth:
    ...
    st.markdown(f"<h1 style='color:#f59e0b; font-size:42px;'>MA3LOMATI</h1><p style='color:#aaa;'>{L['login_h']}</p>", unsafe_allow_html=True)
    u_in = st.text_input(L["user"], placeholder="Username")
    p_in = st.text_input(L["pass"], type="password", placeholder="Password")
...
if menu_selection in ["Tools", "الأدوات"]:
    ...
    with t1:
        with st.container(border=True):
            ...
            p_v = st.number_input("Amount", 0)
            y_v = st.number_input("Years", 1, 20, 7)
            if p_v > 0: st.warning(f"Monthly: {p_v/(y_v*12):,.2f}")
        with st.container(border=True):
            st.subheader("📏 Area")
            m_v = st.number_input("SQM", 0.0)
            st.info(f"SQFT: {m_v * 10.76:.2f}")
    with t2:
        with st.container(border=True):
            st.subheader("📈 ROI")
        ...
        with st.container(border=True):
            ...
            v_v = st.number_input("Deal Value", 0); perc = st.slider("%", 1.0, 5.0, 2.5)
            st.info(f"Earn: {v_v*(perc/100):,.0f}")
    with t3:
        with st.container(border=True):
            st.subheader("🌍 Currency")
            u_v = st.number_input("USD Amount", 0.0); rate = st.number_input("Rate", 40.0, 70.0, 50.0)
            st.warning(f"EGP: {u_v*rate:,.2f}")

elif menu_selection in ["AI Assistant", "المساعد الذكي"]:
    st.markdown(f"<div class='tool-card'><h3>🤖 AI Assistant</h3><p>{L['ai_welcome']}</p></div>", unsafe_allow_html=True)
    ...
    elif is_launch: active_df, col_n = df_l, 'Project'
    ...
    if st.session_state.view == "details":
        ...
        for i, col_set in enumerate([cols[:split], cols[split:split*2], cols[split*2:]]):
            with [c1, c2, c3][i]:
        ...
        filtered = active_df[active_df[active_df.columns[0]].astype(str).str.contains(search, case=False)]
        ...
        with c_main:
            grid = st.columns(2)
            for i, (orig_idx, r) in enumerate(display.iterrows()):
                with grid[i % 2]:
                    if st.button(f"✨ {r[0]}\n📍 {r.get('Area','---')}\n🏢 {r.get('Developer','---')}", key=f"card_{orig_idx}"):
                        st.session_state.current_index, st.session_state.view = orig_idx, "details"; st.rerun()
        with c_side:
            st.markdown(f"<p style='color:#f59e0b; font-weight:bold;'>{L['side_dev'] if menu_selection=='Developers' else L['side_proj']}</p>", unsafe_allow_html=True)
