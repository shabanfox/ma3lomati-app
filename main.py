...
# 2. تصميم CSS (أسود وأبيض مع تخصيص العرض)
st.markdown("""
    <style>
    ...
    .dev-name { font-size: 1.5rem; font-weight: 900; color: #000; }
...
if st.session_state.view == 'main':
    ...
    with mid_col:
        ...
        with c1:
            if st.button("🏢\nدليل المطورين", key="m_dev"): st.session_state.view = 'comp'; st.rerun()
        with c2:
            if st.button("🛠️\nأدوات البروكر", key="m_tool"): st.session_state.view = 'tools'; st.rerun()

elif st.session_state.view == 'comp':
    st.markdown('<div class="hero-banner"><h2>🏢 دليل المطورين العقاريين</h2></div>', unsafe_allow_html=True)
    
    # تقسيم الصفحة: 70% للشبكة و 30% مساحة فارغة
        ...
        for i in range(0, len(current_devs), 3):
            ...
            for j in range(3):
                if i + j < len(current_devs):
                    ...
                    with grid_cols[j]:
                        st.markdown(f'<div class="dev-card"><div class="dev-name">{dev_name}</div></div>', unsafe_allow_html=True)

        # أزرار التنقل ممركزة أسفل الـ 70%
