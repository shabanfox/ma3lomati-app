...
# 8. شريط المعلومات والساعة الذكية (تحديث أوتوماتيكي كل دقيقة بدون ريفريش)
...
with c_top2:
    st.markdown(f"""
        <div style='text-align: left; padding: 5px; color: #aaa; font-size: 14px;'>
            ...
            <span style='cursor:pointer; color:#f59e0b; margin-right:15px;' onclick='window.location.reload()'>🔄</span>
        </div>
        <script>
            ...
            setInterval(updateClock, 60000); // تحديث كل 60 ثانية (دقيقة) لتقليل الحمل
        </script>
    """, unsafe_allow_html=True)
...
# باقي الكود (المشاريع، المطورين، أدوات البروكر) كما هو في نسختك الأصلية...
if st.session_state.selected_item is not None:
    if st.button("⬅️ عودة للقائمة"): st.session_state.selected_item = None; st.rerun()
    item = st.session_state.selected_item
    st.markdown(f"<div class='smart-box'><h2>{item.get('ProjectName', item.get('Developer'))}</h2></div>", unsafe_allow_html=True)
