...
# --- 2. الثوابت والروابط (تم دمج روابط الأكواد السابقة) ---
...
# --- 4. وظائف الربط مع جوجل شيت (الخلفية والتحقق) ---
...
# --- 5. التنسيق الجمالي (CSS) - تصميم 2026 الفاخر ---
st.markdown(f"""
    <style>
...
if not st.session_state.auth:
    ...
    with tab_signup:
        reg_name = st.text_input("الأسم بالكامل", placeholder="الاسم")
        reg_pass = st.text_input("كلمة السر المرجوة", type="password", placeholder="كلمة السر")
        reg_email = st.text_input("الجيميل", placeholder="الإيميل")
        reg_wa = st.text_input("رقم الواتساب", placeholder="رقم الموبايل")
        ...
        if st.button("تأكيد الاشتراك ✅", use_container_width=True):
            if reg_name and reg_pass and reg_email:
                if signup_user(reg_name, reg_pass, reg_email, reg_wa, reg_co):
                    st.success("تم تسجيلك! يمكنك الآن تسجيل الدخول.")
...
menu = option_menu(None, ["أدوات البروكر", "المطورين", "المشاريع", "المساعد الذكي", "Launches"], 
    icons=["briefcase", "building", "search", "robot", "megaphone"], default_index=4, orientation="horizontal",
    styles={"nav-link-selected": {"background-color": "#f59e0b", "color": "black", "font-weight": "bold"}})

# إعادة تصفير الصفحة عند تغيير المنيو
...
if menu == "أدوات البروكر":
    ...
    with r1c3:
        with st.container(border=True):
            ...
            st.metric("ROI", f"{(rent/buy)*100:,.1f}%")

# 10.2 المساعد الذكي
elif menu == "المساعد الذكي":
    st.markdown("<div class='detail-card'><h3>🤖 Real Estate AI Assistant</h3></div>", unsafe_allow_html=True)
    for msg in st.session_state.messages:
        with st.chat_message(msg["role"]): st.markdown(msg["content"])
    if pmt := st.chat_input("اسألني عن السوق العقاري..."):
        st.session_state.messages.append({"role": "user", "content": pmt})
        st.session_state.messages.append({"role": "assistant", "content": "جاري تحليل البيانات... بناءً على أحدث المشاريع المسجلة، ننصح بمتابعة منطقة التجمع الخامس حالياً."})
        st.rerun()

# 10.3 المشاريع، المطورين، Launches
else:
    active_df = df_p if menu=="المشاريع" else (df_l if menu=="Launches" else df_d)
    if active_df.empty: st.error("No Data Found")
    else:
        ...
        # --- VIEW: GRID ---
            ...
            search = st.text_input("🔍 ابحث هنا باسم المشاريع أو المطورين")
