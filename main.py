...
def check_auth(u, p):
    try:
        ...
        # التحقق من تطابق الاسم وكلمة المرور في الشيت
...
# --- 6. Flow Logic (Login First) ---

if not st.session_state.auth:
    st.markdown("<div class='login-box'>", unsafe_allow_html=True)
    st.markdown(f"<h1 style='color:#f59e0b; font-size:42px;'>MA3LOMATI</h1>", unsafe_allow_html=True)
    st.markdown(f"<p style='color:#ccc; margin-bottom:30px;'>{L['login_h']}</p>", unsafe_allow_html=True)
...
# --- 7. Main Platform Content ---
    ...
    # --- بقية منطق العرض (أدوات، مساعد، مشاريع) كما في كودك تماماً ---
    if menu_selection in ["Tools", "الأدوات"]:
        ...
        with t2:
            with st.container(border=True):
                ...
                c_val = st.number_input("Cost", 1); r_val = st.number_input("Annual Rent", 0)
                st.warning(f"ROI: {(r_val/c_val)*100:.2f}%")
        with t3:
            with st.container(border=True):
                st.subheader("🌍 Currency / العملة")
                u_val = st.number_input("USD Amount", 0.0); rate_val = st.number_input("Rate", 40.0, 70.0, 50.0)
        ...
        if prompt := st.chat_input("Ask about market trends..."):
            ...
            st.session_state.messages.append({"role": "assistant", "content": f"Analyzing for: {prompt}"})
            st.rerun()

    else:
        # نظام المشاريع والمطورين (الشبكة والتفاصيل)
        is_launch = menu_selection in ["Launches", "اللونشات"]
        if menu_selection in ["Projects", "المشاريع"]: active_df, col_main_name = df_p, df_p.columns[0]
        elif is_launch: active_df, col_main_name = df_l, df_l.columns[0]
        else: active_df, col_main_name = df_d, df_d.columns[0]
        ...
        if st.session_state.view == "details":
            ...
            filtered = active_df[active_df[col_main_name].astype(str).str.contains(search, case=False)]
            ...
            with c_side:
                st.markdown(f"<p style='color:#f59e0b; font-weight:bold; font-size:14px;'>{L['side_proj']}</p>", unsafe_allow_html=True)
