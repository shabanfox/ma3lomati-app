    ...
    with tab_signup:
        ...
        with c2:
            ...
            if st.button("تأكيد الاشتراك ✅"):
                if signup_user(reg_name, reg_pass, reg_email, reg_wa, reg_co): st.success("تم تسجيلك بنجاح!")
...
# 8. شريط المعلومات العلوي والساعة الحية (Automatic Update)
c_top1, c_top2 = st.columns([0.7, 0.3])
with c_top1:
    st.markdown(f'<div class="ticker-wrap"><div class="ticker">🔥 {news_text}</div></div>', unsafe_allow_html=True)
with c_top2:
    # حقن JavaScript لتحديث الساعة كل ثانية أوتوماتيكياً
    st.markdown(f"""
...
# باقي كود صفحات (المساعد، المشاريع، المطورين، الأدوات) كما هو في كودك الأصلي
elif menu == "المشاريع":
    f1, f2 = st.columns(2)
    search = f1.text_input("🔍 ابحث")
    area_f = f2.selectbox("📍 فلتر", ["الكل"] + sorted(df_p['Location'].unique().tolist()))
