import streamlit as st
import pandas as pd

# 1. إعدادات الصفحة
st.set_page_config(page_title="مساعد البروكر الذكي", layout="wide")

# 2. التنسيق (CSS) - الحفاظ على شكل الكروت الرمادية والاسم يمين
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Cairo:wght@400;700;900&display=swap');
    [data-testid="stHeader"], footer, .stDeployButton, #MainMenu {display: none !important;}
    html, body, [data-testid="stAppViewContainer"] { 
        direction: RTL; text-align: right; font-family: 'Cairo', sans-serif; background-color: #f8fafc; 
    }
    .header-nav { text-align: right; padding: 10px 0; margin-bottom: 5px; }
    .broker-tool-box { 
        background: linear-gradient(135deg, #003366, #00509d); 
        color: white; padding: 25px; border-radius: 15px; margin-bottom: 20px; 
    }
    .project-card-container { 
        background-color: #edf2f7; border-radius: 12px; margin-bottom: 8px; 
        display: flex; align-items: center; border: 1px solid #e2e8f0; 
    }
    div.stButton > button {
        background-color: #003366 !important; color: white !important;
        border-radius: 6px !important; padding: 4px 12px !important;
    }
    </style>
""", unsafe_allow_html=True)

# 3. جلب وتنظيف البيانات
@st.cache_data(ttl=60)
def load_data():
    csv_url = "https://docs.google.com/spreadsheets/d/e/2PACX-1vR7AlPjwOSyd2JIH646Ie8lzHKwin6LIB8DciEuzaUb2Wo3sbzVK3w6LSRmvE4t0Oe9B7HTw-8fJCu1/pub?output=csv"
    try:
        df = pd.read_csv(csv_url)
        df.columns = [str(c).strip() for c in df.columns]
        # تنظيف السعر: إزالة الفواصل والكلمات وتحويله لرقم
        df['Price_Numeric'] = pd.to_numeric(df['Price'].astype(str).str.replace(r'[^\d.]', '', regex=True), errors='coerce').fillna(0)
        # التأكد من وجود أعمدة المقدم والقسط وتحويلها لأرقام
        df['Down_Payment'] = pd.to_numeric(df['Down_Payment'], errors='coerce').fillna(10)
        df['Installments'] = pd.to_numeric(df['Installments'], errors='coerce').fillna(7)
        return df
    except Exception as e:
        st.error(f"خطأ في تحميل البيانات: {e}")
        return None

df = load_data()

if 'page' not in st.session_state: st.session_state.page = 'main'

# --- الصفحة الرئيسية ---
if st.session_state.page == 'main':
    st.markdown('<div class="header-nav"><div style="color:#003366; font-weight:900; font-size:1.8rem;">منصة معلوماتى <span style="color:#D4AF37;">العقارية</span></div></div>', unsafe_allow_html=True)

    st.markdown('<div class="broker-tool-box"><h3>🎯 محرك المطابقة المالية (للمستشار العقاري)</h3><p>أدخل ميزانية العميل وسأقوم بترشيح أفضل المطورين المتاحين</p></div>', unsafe_allow_html=True)
    
    # مدخلات البروكر
    with st.expander("🛠️ أدوات تحليل الميزانية", expanded=True):
        c1, c2, c3 = st.columns(3)
        with c1:
            budget_total = st.number_input("إجمالي ميزانية العميل (ج.م)", min_value=0, value=5000000)
        with c2:
            down_ready = st.number_input("المقدم المتاح حالياً (ج.م)", min_value=0, value=500000)
        with c3:
            monthly_ready = st.number_input("القسط الشهري المتاح (ج.م)", min_value=0, value=40000)

    if df is not None:
        # منطق الفلترة المالي
        def is_match(row):
            price = row['Price_Numeric']
            # لو السعر 0 نتخطى (بيانات ناقصة)
            if price == 0: return False
            
            req_dp = price * (row['Down_Payment'] / 100)
            req_monthly = (price - req_dp) / (row['Installments'] * 12)
            
            # الشروط: السعر الكلي والمقدم والقسط
            return price <= budget_total and req_dp <= down_ready and req_monthly <= monthly_ready

        matches = df[df.apply(is_match, axis=1)]

        st.info(f"النتائج المطابقة لميزانية العميل: {len(matches)}")

        for _, row in matches.iterrows():
            st.markdown('<div class="project-card-container">', unsafe_allow_html=True)
            col_content, col_img = st.columns([4, 1])
            with col_content:
                txt_c, btn_c = st.columns([3, 1])
                with txt_c:
                    dp_val = int(row['Price_Numeric'] * (row['Down_Payment']/100))
                    mo_val = int((row['Price_Numeric'] - dp_val) / (row['Installments'] * 12))
                    st.markdown(f"""
                        <div style="text-align: right; padding: 15px;">
                            <div style="color: #003366; font-weight: 900; font-size: 1.3rem;">{row.get('Developer')}</div>
                            <div style="color: #D4AF37; font-weight: 700;">المقدم: {dp_val:,} ج.م | القسط: {mo_val:,} ج.م</div>
                            <div style="color: #4a5568; font-size: 0.85rem;">📍 {row.get('Area')} | نظام السداد: {int(row['Down_Payment'])}% مقدم على {int(row['Installments'])} سنين</div>
                        </div>
                    """, unsafe_allow_html=True)
                with btn_c:
                    st.write("")
                    if st.button("التفاصيل", key=f"btn_{row.get('Developer')}"):
                        st.session_state.selected_item = row.to_dict()
                        st.session_state.page = 'details'
                        st.rerun()
            with col_img:
                img_url = row.get('Image_URL', 'https://via.placeholder.com/400')
                st.markdown(f'<div style="height: 110px; border-radius: 0 10px 10px 0; background-image: url(\'{img_url}\'); background-size: cover; background-position: center;"></div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)

# --- صفحة التفاصيل ---
elif st.session_state.page == 'details':
    item = st.session_state.selected_item
    st.markdown('<div style="background:white; padding:30px; border-radius:15px; margin-top:10px; border: 1px solid #e2e8f0;">', unsafe_allow_html=True)
    if st.button("⬅️ عودة"):
        st.session_state.page = 'main'
        st.rerun()
    st.markdown(f"<h2>{item.get('Developer')}</h2>", unsafe_allow_html=True)
    st.write(f"الموقع: {item.get('Area')}")
    st.write(f"تفاصيل المشاريع: {item.get('Projects')}")
    st.markdown('</div>', unsafe_allow_html=True)
