...
st.markdown(f"""
    ...
    .ticker-wrap {{ width: 100%; background-color: #1a1a1a; border-bottom: 2px solid #f59e0b; padding: 6px 0; margin-bottom: 15px; overflow: hidden; white-space: nowrap; }}
    .ticker {{ display: inline-block; animation: ticker 40s linear infinite; color: #fff; font-size: 13px; }}
...
with main_col:
    ...
    if menu == T['projects']:
        ...
        if b2.button(T['prev']) and st.session_state.idx_p > 0: st.session_state.p_idx -= 1; st.rerun()
                ...
                if i + j < len(dff_d):
                    ...
                    with cols[j]:
                        ...
                        with st.expander("📖 تفاصيل المطور"):
                            ...
                            st.divider()
                            st.write(f"ℹ️ **عن الشركة:** {row.get('Detailed_Info', 'N/A')}")
    ...
    # --- أدوات البروكر ---
        ...
        st.metric("القسط الشهري التقديري", f"{p/(y*12):,.0f} ج.م")
