...
# 2. كود التصميم (CSS) - الشكل الرمادي الهادئ المعتمد
st.markdown("""
    <style>
...
if st.session_state.page == 'main':
    ...
    if df is not None:
        ...
        with c3:
            s_price = st.selectbox("💰 السعر", ["الكل", "أقل من 5 مليون", "10 مليون+"])
                ...
                with txt_c:
                    st.markdown(f"""
                        <div style="text-align: right; padding: 15px;">
                            ...
                            <div style="color: #64748b; font-size: 0.8rem;">📍 {row.get('Area')} | {row.get('Price')}</div>
                        </div>
                    """, unsafe_allow_html=True)
                with btn_c:
                    st.write("")
