...
# 6. جلب البيانات مع تحديث كل 200 ثانية
...
with main_col:
    if st.session_state.selected_item is not None:
        ...
        st.markdown(f'<div style="background:#111; padding:25px; border-radius:15px; border-right:5px solid #f59e0b; color:white;"><h2>{item.get("Project Name", item.get("Developer"))}</h2><hr opacity="0.1"><p style="font-size:18px;">{item.get("Project Features", item.get("Detailed_Info", "لا يوجد تفاصيل"))}</p></div>', unsafe_allow_html=True)
        ...
        if f4.button("🔄", key="refresh_btn", help="تحديث البيانات من الشيت الآن"):
            st.cache_data.clear()
            st.rerun()
        ...
        with t1:
            ...
            st.metric("القسط الربع سنوي", f"{rem/(years*4):,.0f} ج.م")
        with t2:
            sqm = st.number_input("المساحة بالمتر المربع", value=100.0)
            st.write(f"القدم المربع: {sqm * 10.76:,.2f}")
            st.write(f"الفدان: {sqm / 4200:,.4f}")import streamlit as st
import pandas as pd
import feedparser
from datetime import datetime
from streamlit_option_menu import option_menu
