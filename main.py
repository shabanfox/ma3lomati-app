...
# 4. التنسيق البصري (ألوان 2026 الاحترافية)
st.markdown(f"""
<style>
    ...
    p, span, label {{ color: #FFFFFF !important; font-size: 16px; font-weight: 700; }}

    /* كروت المشاريع - خلفية بيضاء وخط أسود عريض جداً للوضوح */
...
if st.session_state.selected_item is not None:
    ...
    if st.session_state.p_idx > 0:
        if col1.button("السابق"):
            st.session_state.p_idx -= 1
            st.rerun()
    if start + 6 < len(dff):
        if col2.button("التالي"):
            st.session_state.p_idx += 1
            st.rerun()
        ...
        if st.button("تأكيد الاشتراك ✅"):
            if reg_name and reg_pass and reg_email:
                if signup_user(reg_name, reg_pass, reg_email, "---", "---"):
                    st.success("تم تسجيلك بنجاح! اذهب لتسجيل الدخول.")
...
if st.session_state.selected_item is not None:
    ...
    item = st.session_state.selected_item
    st.markdown(f"<div class='smart-box'><h2>{item.get('ProjectName', item.get('Developer'))}</h2><p>📍 الموقع: {item.get('Location')}</p><p>🏗️ المطور: {item.get('Developer')}</p><p>💰 السعر: {item.get('Starting Price (EGP)')}</p></div>", unsafe_allow_html=True)
    ...
    with c1:
        st.markdown("<div class='tool-card'><h3>💳 القسط</h3>", unsafe_allow_html=True)
        total = st.number_input("السعر", value=1000000)
...
st.markdown("<p style='text-align:center; color:#444; margin-top:50px;'>MA3LOMATI PRO © 2026 | النسخة الاحترافية</p>", unsafe_allow_html=True)                else:
                    user_verified = login_user(u_input, p_input)
                    if user_verified:
                        st.session_state.auth = True
                        st.session_state.current_user = user_verified
...
st.markdown("<p style='text-align:center; color:#444; margin-top:50px;'>MA3LOMATI PRO © 2026 | النسخة الاحترافية</p>", unsafe_allow_html=True)    </style>
""", unsafe_allow_html=True)
