...
# --- 5. دالة العرض المحدثة (الفلاتر الاحترافية) ---
def render_grid(dataframe, prefix):
    if st.session_state.view == f"details_{prefix}":
        ...
        with f2:
            # فلتر المناطق ديناميكي من الشيت
            if 'Location' in dataframe.columns:
                unique_locs = ["الكل"] + sorted([str(x).strip() for x in dataframe['Location'].unique() if str(x).strip() not in ["---", "nan", ""]])
            else: unique_locs = ["الكل"]
            sel_area = st.selectbox("📍 المنطقة", unique_locs, key=f"l_{prefix}")
            
        with f3:
            # فلتر السعر ذكي
            if 'Price' in dataframe.columns:
                dataframe['Price'] = pd.to_numeric(dataframe['Price'].astype(str).str.replace(r'[^\d]', '', regex=True), errors='coerce').fillna(0)
                min_p, max_p = int(dataframe['Price'].min()), int(dataframe['Price'].max())
                if min_p == max_p: max_p += 1
                price_range = st.slider("💰 الميزانية", min_p, max_p, (min_p, max_p), key=f"p_{prefix}")
        ...
        with m_c:
            ...
            for i, (idx, r) in enumerate(disp.iterrows()):
                with grid[i%2]:
                    # استخراج بيانات الكارت بشكل مرن
            ...
            with p_info: st.markdown(f"<p style='text-align:center; color:#f59e0b; font-weight:bold;'>صفحة {st.session_state.page_num + 1}</p>", unsafe_allow_html=True)
            with p2:
...
st.markdown(f"""
    ...
    div.stButton > button[key*="card_"] {{ background: #fff !important; color: #1a1a1a !important; border-right: 10px solid #f59e0b !important; border-radius: 15px !important; text-align: right !important; min-height: 160px !important; font-weight: bold !important; font-size: 1.1rem !important; }}
    .detail-card {{ background: rgba(20, 20, 20, 0.95); padding: 25px; border-radius: 20px; border-top: 6px solid #f59e0b; border: 1px solid #333; }}
    .label-gold {{ color: #f59e0b; font-weight: 900; font-size: 1.1rem; }}
    .val-white {{ color: white; font-size: 1.3rem; font-weight: bold; border-bottom: 1px solid #333; margin-bottom: 10px; padding-bottom: 5px; }}
...
if menu == "أدوات البروكر":
    st.markdown("<h2 style='color:#f59e0b; text-align:center; font-weight:900;'>🛠️ أدوات الحساب</h2>", unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    with c1:
        ...
        st.markdown(f"<p class='label-gold'>القسط الشهري:</p><p style='font-size:24px; font-weight:900;'>{res:,.0f} ج.م</p></div>", unsafe_allow_html=True)
    with c2:
        ...
        st.markdown(f"<p class='label-gold'>العمولة الصافية:</p><p style='font-size:24px; font-weight:900;'>{deal*(pct/100):,.0f} ج.م</p></div>", unsafe_allow_html=True)
    with c3:
        ...
        st.markdown(f"<p class='label-gold'>العائد السنوي:</p><p style='font-size:24px; font-weight:900;'>{roi:.2f} %</p></div>", unsafe_allow_html=True)
    ...
    st.info("نظام AI 2026 قيد التحليل")

# زر الخروج
st.sidebar.write(f"المستخدم: {st.session_state.current_user}")
if st.sidebar.button("🚪 تسجيل الخروج"): logout()

st.markdown("<p style='text-align:center; color:#555; font-weight:bold; margin-top:50px;'>MA3LOMATI PRO © 2026</p>", unsafe_allow_html=True)
