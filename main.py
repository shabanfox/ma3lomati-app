...
if 'last_menu' not in st.session_state: st.session_state.last_menu = "المساعد الذكي"
...
# 9. المنيو الرئيسي (تم إضافة كود حل مشكلة التنقل هنا)
...
# كود تصفير الاختيار عند تغيير القائمة لضمان الانتقال السلس
if menu != st.session_state.last_menu:
    st.session_state.selected_item = None
    st.session_state.last_menu = menu
    st.rerun()

# 10. تفاصيل المشروع أو المطور (صفحة منبثقة)
if st.session_state.selected_item is not None:
    ...
    st.markdown(f"""<div class='smart-box'>
        ...
        <hr><p>{item.get('Payment Plan', item.get('Detailed_Info', 'خطط سداد متنوعة متاحة عند التواصل'))}</p>
