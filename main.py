...
if 'user_db' not in st.session_state: st.session_state.user_db = {"admin": "2026"} # قاعدة بيانات للمستخدمين
...
# 5. شاشة الدخول والتسجيل (تم التحديث هنا فقط)
if not st.session_state.auth:
    ...
    with c2:
        mode = st.radio("اختر العملية", ["تسجيل دخول", "إنشاء حساب جديد"], horizontal=True)
        ...
        if mode == "تسجيل دخول":
            ...
            new_pwd = st.text_input("اختر كلمة سر", type="password")
            if st.button("تأكيد التسجيل ✅", use_container_width=True):
                if new_user and new_pwd:
                    st.session_state.user_db[new_user] = new_pwd
                    st.success("تم الحفظ! يمكنك الآن تسجيل الدخول")
                else:
                    st.warning("برجاء إدخال بيانات صحيحة")
    st.stop()
