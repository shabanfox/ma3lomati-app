...
trans = {
    "EN": {
        ...
        "det_title": "Specifications", "ai_welcome": "How can I help you today?",
    ...
    "AR": {
        ...
        "det_title": "المواصفات والتفاصيل", "ai_welcome": "كيف يمكنني مساعدتك اليوم؟",
...
st.markdown(f"""
    ...
    .info-box {{
        margin-bottom: 20px; padding: 15px; border-right: 4px solid #f59e0b;
    ...
    .val-white {{ color: white; font-size: 19px; line-height: 1.6; }}
    </style>
...
if menu_selection in ["Tools", "الأدوات"]:
    ...
    st.markdown(f"<div class='detail-card'><h3>🤖 MA3LOMATI AI</h3><p>{L['ai_welcome']}</p></div>", unsafe_allow_html=True)
    ...
    if st.session_state.view == "details":
        ...
        # --- صفحة التفاصيل المقسمة بشكل فخم ---
        st.markdown(f'<div class="detail-card"><h1 style="color:#f59e0b;">{item[col_main_name]}</h1><hr>', unsafe_allow_html=True)
        
        for label, data_col in mapping.items():
            val = item.get(data_col, "---")
            st.markdown(f"""
                <div class="info-box">
        ...
        if is_launch:
            ...
            with col_main:
                ...
                for i, (orig_idx, r) in enumerate(display_df.iterrows()):
                    with grid[i % 2]:
                        if st.button(f"✨ {r[col_main_name]}\n📍 {r.get('Area', r.get('Owner / Chairman', 'Egypt'))[:30]}...", key=f"card_{orig_idx}"):
                            st.session_state.current_index = orig_idx; st.session_state.view = "details"; st.rerun()
            with col_side:
