...
# 2. التنسيق (CSS) - المحاذاة والجماليات
st.markdown("""
    <style>
...
if not st.session_state.auth:
    ...
    pwd = st.text_input("أدخل الباسورد للدخول", type="password")
...
df = load_data()
grid_size = 9 # تعريف المتغير عالمياً لتجنب NameError
...
selected_menu = option_menu(None, ["🛠️ أدوات البروكر", "🏗️ المشاريع", "🏢 المطورين"], 
                           icons=["tools", "building", "person-vcard"], orientation="horizontal")

# --- 🏗️ قسم المشاريع ---
if selected_menu == "🏗️ المشاريع":
    ...
    if b1.button("➡️ السابق") and st.session_state.p_idx > 0:
        st.session_state.p_idx -= 1; st.rerun()
    if b2.button("التالي ⬅️") and st.session_state.p_idx < total_p - 1:
        st.session_state.p_idx += 1; st.rerun()
...
# --- 🏢 قسم المطورين ---
    ...
    if db1.button("➡️ السابق") and st.session_state.d_idx > 0:
        st.session_state.d_idx -= 1; st.rerun()
    if db2.button("التالي ⬅️") and st.session_state.d_idx < total_d - 1:
        st.session_state.d_idx += 1; st.rerun()
    st.markdown(f"<p style='text-align:right;'>صفحة {st.session_state.d_idx + 1} من {total_d}</p>", unsafe_allow_html=True)

# --- 🛠️ الأدوات ---
elif selected_menu == "🛠️ أدوات البروكر":
    ...
    with c2:
        ...
        st.text_area("الملاحظات...", height=150)
        st.markdown("</div>", unsafe_allow_html=True)
