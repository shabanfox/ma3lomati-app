...
# --- 3. Session State & Language Dictionary ---
if 'auth' not in st.session_state: st.session_state.auth = False
if 'lang' not in st.session_state: st.session_state.lang = "AR" # Default Arabic
...
# --- 4. Professional CSS (Dark Theme + Bold White Text) ---
st.markdown(f"""
    <style>
    ...
    /* Minimalist Card Design: Bold White Text - Center Aligned */
    ...
    .info-card {{ background: rgba(255,255,255,0.03); padding: 30px; border-radius: 20px; border: 1px solid #333; }}
    .label-gold {{ color: #f59e0b; font-weight: 900; font-size: 14px; margin-bottom: 5px; }}
...
st.markdown(f"""
    <div class="royal-header">
        <h1 style="color: #f59e0b; font-size: 55px; margin: 0; font-weight: 900;">{L['title']}</h1>
        <p style="color: #fff; letter-spacing: 4px;">{L['subtitle']}</p>
    </div>
""", unsafe_allow_html=True)

# Top Bar (Menu | Language | Logout)
...
if st.session_state.selected_item is not None:
    ...
    with c1:
        st.markdown(f"""<div class="info-card">
            ...
            <p class="label-gold">{L['fields'][3]}</p><p style="font-size:20px;">{it.get('Unique Selling Points (USP)', it.get('Notes','---'))}</p>
    ...
    # --- Shared View Logic: Show Name Only ---
