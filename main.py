...
# 1. إعدادات النظام وتصميم الواجهة (CSS)
...
st.markdown("""
    ...
    .grid-card {
        ...
        padding: 25px;
        margin-bottom: 15px;
        min-height: 350px; /* طول مرن للكارت */
        height: auto;
        transition: 0.3s all;
        color: white;
        display: flex;
    ...
    /* تعديل حجم خط المشاريع والمطورين - كبير وبولد */
    .card-title { 
        color: #f59e0b; 
...
if selected == "🛠️ أدوات البروكر":
    ...
    with c1:
        ...
        dp_pct = st.number_input("المقدم %", value=10)
        dp_val = (dp_pct/100)*p
        remain = p - dp_val
...
# --- 2. شاشة المشاريع ---
    ...
    if not df.empty:
        ...
        with f1: s_p = st.text_input("🔍 ابحث عن المشروع...")
    ...
    if not df.empty and 'Developer' in df.columns:
                    ...
                    with cols[j]:
                        st.markdown(f"""
                            <div class="grid-card">
                                ...
                                <div style="font-size:14px; color:#bbb; flex-grow:1;">
                                    <b>نبذة:</b><br>{str(row.get('Detailed_Info', 'لا توجد تفاصيل'))[:180]}...
                                </div>
                            </div>
                        """, unsafe_allow_html=True)
        ...
        with b1: 
            if st.session_state.pg_d > 1:
                if st.button("⬅️ السابق"): st.session_state.pg_d -= 1; st.rerun()
        with b2: st.markdown(f"<p style='text-align:center;'>صفحة {st.session_state.pg_d} من {pages_d}</p>", unsafe_allow_html=True)
        with b3:
            if st.session_state.pg_d < pages_d:
                if st.button("التالي ➡️"): st.session_state.pg_d += 1; st.rerun()
