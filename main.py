...
# --- 2. روابط البيانات ---
# رابط الـ Apps Script الخاص بك (للتسجيل والدخول)
...
st.markdown(f"""
    ...
    .smart-box {{ background: #111; padding: 25px; border-radius: 20px; border-right: 6px solid #f59e0b; color: white; }}
    </style>
""", unsafe_allow_html=True)

# --- 6. شاشة الدخول (المرتبطة بالشيت) ---
    ...
    with col_mid:
        ...
        if st.button("دخول للمنصة 🚀", use_container_width=True):
            if p_input == "2026": # كود طوارئ
                ...
                else: st.error("بيانات الدخول غير صحيحة أو السيرفر لا يستجيب")
    st.stop()
...
if menu == "اللونشات":
    if st.session_state.selected_item is not None:
        ...
        st.markdown(f"<div class='smart-box'><h1>{it.get('Project')}</h1><p>🏢 {it.get('Developer')}</p><p>📍 {it.get('Location')}</p><hr><h3>USP:</h3><p>{it.get('Unique Selling Points (USP)')}</p></div>", unsafe_allow_html=True)
    else:
    ...
    with s_col: st.markdown("<div class='smart-box'><h4>📌 مناطق ساخنة</h4><p>التجمع<br>أكتوبر<br>الساحل</p></div>", unsafe_allow_html=True)
    with m_col:
    ...
    # جعل صفحة المطورين مثل صفحة المشاريع (70/30)
    m_col_d, s_col_d = st.columns([0.7, 0.3])
    with s_col_d:
        st.markdown("<div class='smart-box'><h4>🏆 تصنيف المطورين</h4><p>الفئة A+: عمالقة السوق<br>الفئة A: مطورين موثوقين</p></div>", unsafe_allow_html=True)
    with m_col_d:
        ...
        for i, r in dfd.iloc[start_d:start_d+6].reset_index().iterrows():
            with grid_d[i % 2]:
                if st.button(f"🏗️ {r['Developer']}\n⭐ الفئة: {r.get('Developer Category','A')}\n💼 المالك: {r.get('Owner','---')}", key=f"card_d_{i}"):
                    st.info(f"عرض ملف المطور: {r['Developer']}") # يمكن إضافة صفحة تفاصيل لاحقاً
        if len(dfd) > 6:
            c1, c2 = st.columns(2)
    ...
    with c1:
        st.markdown("<div class='smart-box'><h3>💳 القسط</h3><input type='number' placeholder='السعر'></div>", unsafe_allow_html=True)
    with c2:
        st.markdown("<div class='smart-box'><h3>💰 العمولة</h3><input type='number' placeholder='النسبة'></div>", unsafe_allow_html=True)
    with c3:
        st.markdown("<div class='smart-box'><h3>📐 المساحة</h3><input type='number' placeholder='بالمتر'></div>", unsafe_allow_html=True)
