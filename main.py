...
# تفعيل التحديث التلقائي كل ثانية لضمان تحديث الساعة
...
def get_real_news():
    try:
        ...
        return "  •  ".join(news) if news else "Real Estate News: Updates on the way."
    except: return "MA3LOMATI PRO: Your Professional Real Estate Platform 2026."
...
# 5. شاشة الدخول والاشتراك (English)
        ...
        with c2:
            ...
            if st.button("CREATE ACCOUNT ✅"):
                if reg_name and reg_pass and reg_email:
                    ...
                    else: st.error("Connection error with server.")
...
# 8. شريط المعلومات (ساعة متغيرة كل ثانية)
        ...
        if not res.empty:
            for idx, r in res.head(6).iterrows():
                with st.container(border=True):
                    ...
                    msg = urllib.parse.quote(f"أرشح لك مشروع {r['ProjectName']} في {r['Location']}.")
                    c_btn.markdown(f"[📲 إرسال للعميل](https://wa.me/{client_wa}?text={msg})")
